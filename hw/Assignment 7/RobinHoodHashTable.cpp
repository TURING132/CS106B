...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    if(curSize == allocatedSize) return false;
    int index = hashFunc(elem);

    int dis = 0;
    string elemValue = elem;
    for(int _ = 0; _ < allocatedSize; _++){
        if(elems[index].distance == EMPTY_SLOT ){
            elems[index].value = elemValue;
            elems[index].distance = dis;
            curSize ++;
            return true;
        }else{
            // slot filled
            if(dis > elems[index].distance ){
                swap(dis, elems[index].distance);
                swap(elemValue, elems[index].value);
            }
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    for(int _ = 0; _ < allocatedSize; _++){
        if(elems[index].distance == EMPTY_SLOT ||
            ...
            int shift_index = (index + 1) % allocatedSize;
            for(int __ = 0; __ < allocatedSize; __ ++){
                if(elems[shift_index].distance == EMPTY_SLOT
                    || elems[shift_index].distance == 0){
                    break;
                }else{
                    elems[index].distance = elems[shift_index].distance - 1;
                    elems[index].value = elems[shift_index].value;
                    elems[shift_index].distance = EMPTY_SLOT;
                    shift_index = (shift_index + 1) % allocatedSize;
                    index = (index + 1) % allocatedSize;
                }
            }
            curSize --;
