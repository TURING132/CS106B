...
void deleteNucleotides(Nucleotide* dna) {
    ...
    Nucleotide * temp;
    // first, delete cur, next next->next ...
    while(cur != nullptr){
        temp = cur->next;
        delete cur;
        cur = temp;
    }
    // next delete prev, prev->prev
    cur = prev;
    while(cur != nullptr){
        temp = cur->prev;
        delete cur;
...
Nucleotide* toStrand(const string& str) {
    ...
    Nucleotide dummy_head;
    dummy_head.next = nullptr;
    // dummy_head.nex
    Nucleotide * tail = & dummy_head;
    for(auto c : str){
 ...
 */
bool match(Nucleotide * dna, Nucleotide * target){
    Nucleotide* ptr1 = dna, * ptr2 = target;
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    Nucleotide * cur = dna;
    while(cur != nullptr){
        if(match(cur, target)) return cur;
        cur = cur->next;
    }
    return nullptr;
}
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    while(cur != nullptr){
        if(match(cur, target)){
            ...
            if(prev == nullptr && next != nullptr){// delete head
                dna = next;
                dna->prev = nullptr;
            }else if(next == nullptr && prev != nullptr){// delete tail
                prev->next = nullptr;
