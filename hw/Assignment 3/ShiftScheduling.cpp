...
bool overlap(const Set<Shift>& res, Shift target){
    for(auto s : res){
        if(overlapsWith(s, target)){
            return true;
        }
    }
    return false;
...
double getTotalValue(const Set<Shift> & shifts){
    double sum = 0;
    for(auto s : shifts){
        sum += valueOf(s);
    }
...
Set<Shift> getHeightValue(Set<Shift> sofar, Set<Shift> tochoose, int maxHours){
    if(tochoose.size() == 0 || maxHours == 0) return sofar;
    auto top = tochoose.first();
    tochoose.remove(top);
    auto noTop = getHeightValue(sofar, tochoose, maxHours);
    if(lengthOf(top) <= maxHours && ! overlap(sofar, top)){
        // if top can be added, choose higher between withtop and notop
        sofar.add(top);
        auto withTop = getHeightValue(sofar, tochoose, maxHours - lengthOf(top));
        if(getTotalValue(noTop) > getTotalValue(withTop)){
            // shift without the top
            return noTop;
        }else{
            // shift with the top
            return withTop;
        ...
        // if top can't be added, choose notop
        return noTop;
    }
}
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    if(maxHours < 0) error("maxHours should not be negative!");
    Set<Shift> empty;
    auto res = getHeightValue(empty, shifts, maxHours);
    return res;
}
