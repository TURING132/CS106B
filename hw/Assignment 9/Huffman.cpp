 ...
 */
EncodingTreeNode* createNode(char ch){
...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    if(map.size() < 2) {error("a least two different char!"); return nullptr;}
    PriorityQueue<pair<EncodingTreeNode *, int>> pq;
    for(auto & c : map.keys()){
        auto newNode = createNode(c);
        pq.enqueue(pair(newNode, map[c]), map[c]);
    }
    while(pq.size() > 1){
        auto newNode = createNode(char(0));
        auto zero = pq.dequeue();
        auto one = pq.dequeue();
        newNode->zero = zero.first;
        newNode->one = one.first;
        int newWeight = zero.second + one.second;
        pq.enqueue(pair(newNode, newWeight), newWeight);
    }
...
bool isLeave(EncodingTreeNode * node){
    if(node->one == nullptr && node->zero == nullptr) return true;
    else return false;
}
...
void recDfs(EncodingTreeNode * root, Map<char, string> & map, string s){
    if(root == nullptr) return;
    if(isLeave(root)) map[root->ch] = s;
    recDfs(root->one, map, s + "1");
    recDfs(root->zero, map, s + "0");
}
...
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    ...
    for(auto & ch : str){
        for(auto & c : map[ch]){
            if(c == '0') result.enqueue(0);
            else result.enqueue(1);
...
EncodingTreeNode* decodeTree(Queue<Bit>& bits, Queue<char>& leaves) {
    if(bits.isEmpty()) return nullptr;
    EncodingTreeNode * newNode = new EncodingTreeNode;
    newNode->one = nullptr;
    newNode->zero = nullptr;
    Bit b = bits.dequeue();
    if(b == 0) newNode->ch = leaves.dequeue();
    else{
...
HuffmanResult compress(const string& text) {
    ...
    res.messageBits =  encodeText(text, huffmanTree);
    encodeTree(huffmanTree, res.treeBits, res.treeLeaves);
