...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    // base case, if no possibleLinks for any node, fail
    for(auto & node : possibleLinks){
        if(possibleLinks[node].size() == 0){
            return false;
        }
    }
    // base case, if no more node to match, success
    if(possibleLinks.size() == 0) return true;
    // recursive, pick a node and try all possible link
    string picked = possibleLinks.firstKey();
    for(const string & partner : possibleLinks[picked]){
        Map<string, Set<string>> newPossibleLinks = possibleLinks; // copy
        // remove picked node and chosen partner
        newPossibleLinks.remove(picked);
        newPossibleLinks.remove(partner);
        // remove all edge connected with picked or partner
        for(const string & key : newPossibleLinks){
            Set<string> newValues = newPossibleLinks[key];
            for(const string & p : newPossibleLinks[key]){
                if(p == picked || p == partner){
                    newValues.remove(p);
                }
            }
            newPossibleLinks[key] = newValues;
        }
        matching.add({picked, partner});
        if(hasPerfectMatching(newPossibleLinks, matching)){
            return true;
        }
        // remove pair for new choice
        matching.remove({picked, partner});
