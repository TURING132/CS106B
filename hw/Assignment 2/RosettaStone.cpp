...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if(kGramLength <= 0) {error("error! kGramLength in kGramsIn must be positive"); return map;}
    if(str.size() < kGramLength) return map;
...
Map<string, double> topKGramsIn(const Map<string, double>& source, const int numToKeep) {
    Map<string, double> map;
