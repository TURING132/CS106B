...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    if(numSeats < populations.size()){error("not enough numSeats!"); return {};}
    HeapPQueue queue;
    Map<string, int> res;
    for(auto & province : populations.keys()){
        queue.enqueue(DataPoint(province, - populations[province] / sqrt(2)));
        res[province] = 1;
    }
    for(int _ = 0; _ < numSeats - populations.size(); _ ++){
        DataPoint province = queue.dequeue();
        res[province.name] ++;
        province.weight =  - populations[province.name] /
                            sqrt(res[province.name] * (res[province.name] + 1));
