...
void HeapPQueue::swap(DataPoint & a, DataPoint & b){
    auto temp = a;
    a = b;
    b = temp;
}
...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    while(cur > 1 && elems[cur].weight < elems[cur / 2].weight){
        swap(elems[cur], elems[cur/2]);
        cur /= 2;
    }
}

DataPoint HeapPQueue::peek() const {
    if(logicalSize == 0) {error("no elements!"); return {};}
    return elems[1];
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    if(logicalSize == 0) {error("no elements!"); return {};}
    DataPoint top = elems[1];
    elems[1] = elems[logicalSize];
    logicalSize -- ;

    int cur = 1;
    while(cur * 2 <= logicalSize){
        int min_child_index = cur * 2;
        if(min_child_index + 1 <= logicalSize &&
            elems[min_child_index + 1].weight < elems[min_child_index].weight){
            min_child_index = min_child_index + 1;
        }
        if(elems[cur].weight > elems[min_child_index].weight){
            swap(elems[cur], elems[min_child_index]);
            cur = min_child_index;
        }else{
            break;
        }
